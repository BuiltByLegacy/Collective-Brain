...
import { snapshotEvidence } from './freshness.mjs';
...
export function evidenceBundle(corpus,query,identity,{readAt=new Date(),freshnessPolicy={}}={}){ const results=search(corpus,query,identity,{limit:6}); const evidence=results.map(({artifact,relevance})=>({id:artifact.id,title:artifact.title,revision:artifact.revision,type:artifact.type,authority:artifact.authority,status:artifact.status,relevance,locations:artifact.locations,relationships:artifact.relationships.map(normalizeRelation),classification:artifact.type==='released_procedure'||artifact.type==='licensed_standard_metadata'?'requirement_or_reference':artifact.type==='approved_decision'?'approved_practice':artifact.type==='approved_exemplar'?'precedent_example':'supporting_context',freshness:snapshotEvidence(artifact,{readAt,policy:freshnessPolicy})})); const ids=new Set(evidence.map(e=>e.id)); const conflicts=[]; for(const e of evidence) for(const r of e.relationships) if(r.type==='CONFLICTS_WITH'&&ids.has(r.target)) conflicts.push({from:e.id,to:r.target,assertion:r.assertion}); return {query,identity:identity.id,evidence,conflicts}; }

export function compareAuthority(a,b){ return a.authority===b.authority?'equal':a.authority>b.authority?a.id:b.id; }
export function createProposal({statement,sources,proposer}){ return {id:`PROP-${crypto.randomUUID()}`,kind:'knowledge_proposal',ownership:'brain_owned',authority:'machine_generated',statement,sources,proposer,status:'pending_review',createdAt:new Date().toISOString()}; }
export function approveProposal(proposal,reviewer){return {...proposal,status:'approved',reviewer,reviewedAt:new Date().toISOString(),promotedByHuman:true};}
export function rejectProposal(proposal,reviewer){return {...proposal,status:'rejected',reviewer,reviewedAt:new Date().toISOString(),promotedByHuman:false};}

export function groundedAnswer(bundle){ if(!bundle.evidence.length)return{answer:'No authorized institutional evidence was found. Do not infer company guidance.',sources:[]}; const sorted=[...bundle.evidence].sort((a,b)=>b.authority-a.authority||b.relevance-a.relevance); const top=sorted[0],exemplar=sorted.find(e=>e.type==='approved_exemplar'); const lines=[]; if(exemplar)lines.push(`The current approved exemplar is ${exemplar.title} Rev ${exemplar.revision}; it demonstrates the documented seed-part approach.`); if(top.type==='released_procedure')lines.push(`The highest-authority retrieved company guidance is ${top.title} Rev ${top.revision}.`); if(exemplar)lines.push('Treat the seed part as an approved example/precedent, not as the standards requirement itself.'); if(bundle.conflicts.length)lines.push('Conflicting lower-authority working material exists and should not override current approved guidance.'); return{answer:lines.join(' '),sources:sorted.slice(0,4).map(e=>({id:e.id,title:e.title,revision:e.revision,locations:e.locations.map(l=>l.anchor),authority:e.authority,classification:e.classification,readAt:e.freshness?.readAt??null,sourceVersion:e.freshness?.sourceVersion??e.revision,sourceModifiedAt:e.freshness?.sourceModifiedAt??null,staleAfter:e.freshness?.staleAfter??null}))}; }
